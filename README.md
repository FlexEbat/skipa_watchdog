...
- **`poll`** (по умолчанию) — периодический опрос активных TCP-соединений
  напрямую из `/proc/net/tcp`/`/proc/net/tcp6` (без сторонних библиотек).
  Не требует прав root для самого мониторинга (но они всё равно нужны для
  блокировки). Может пропустить очень короткое соединение между двумя
  опросами.
