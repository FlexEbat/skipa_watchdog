...
Дальше ставит venv и нужные зависимости (см. раздел "Зависимости" ниже —
сколько весит каждый вариант), при выборе варианта 2 сразу же спросит
токен и chat_id, поставит systemd-юнит `skipa-watchdog` и запустит его.
Один раз, сразу после установки, проверит систему на Docker/Kubernetes и
предложит донастроить для них правила (см. раздел ниже) — повторно это
не спрашивается, но доступно из меню в любой момент.
...
Подключить Telegram можно и позже, в любой момент, из меню (пункт 4) — пакет
доустановится сам по требованию.

### Сколько это реально весит

Замерено на пустом venv (`python3 -m venv` + `pip install`):

| | Место на диске (venv) | Память процесса (RSS) |
|---|---|---|
| Только сервис | ~16 МБ | ~25–30 МБ |
| Сервис + Telegram-бот | ~29 МБ | ~38–45 МБ |

Разницу целиком даёт `python-telegram-bot` и его зависимости (`httpx`,
`anyio`, `APScheduler` и т.д.) — то есть Telegram-слой добавляет примерно
**+13 МБ на диске и +12–15 МБ в памяти**. Для сравнения, у типичной
VPS-инстанции это доли процента доступной памяти, но если сервер совсем
маленький (или их много) — вариант "только сервис" ощутимо легче.
